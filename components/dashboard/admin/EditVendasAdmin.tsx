...
export function EditVendasAdmin({ usuarios, grupos: gruposIniciais = [] }: EditVendasProps) {
        ...
        try {
            const res = await fetch('/api/admin/vendas/update', {
                ...
                body: JSON.stringify({
                    ...
                    capitulo: parseFloat(editForm.capitulo),
    ...
    const handleQuickCreate = async (venda: any) => {
        let nextCap = parseFloat(venda.capitulo) + 1;
    ...
    const handleBulkUpdate = async (data: any) => {
        ...
        try {
            const res = await fetch('/api/admin/vendas/bulk', {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (!res.ok) throw new Error();
            toast.success(`${selectedIds.size} registros atualizados!`);
            setSelectedIds(new Set());
            fetchHistory(selectedProduct.produto_id, currentPage);
        } catch (error) {
    ...
    const handleBulkDelete = async () => {
        if (selectedIds.size === 0) return;
        if (!confirm(`Deseja EXCLUIR PERMANENTEMENTE ${selectedIds.size} registros?`)) return;
        
        setIsBulkUpdating(true);
        try {
            const ids = Array.from(selectedIds).join(',');
            const res = await fetch(`/api/admin/vendas/bulk?ids=${ids}`, { method: 'DELETE' });
            if (!res.ok) throw new Error();
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-zinc-950 p-6 rounded-3xl border border-white/5 shadow-2xl space-y-4">
                <div className="flex flex-col md:flex-row gap-4 items-end">
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase italic text-zinc-500 tracking-widest">Modo</label>
                        <Button 
                            onClick={() => {
                            ...
                            className={cn(
                                "h-11 rounded-xl font-black uppercase text-[10px] italic transition-all px-4",
                                isBulkMode ? "bg-amber-500 text-black hover:bg-amber-600" : "border-white/10 text-zinc-400"
                            )}
                        >
                            {isBulkMode ? <Zap className="h-3 w-3 mr-2 fill-current" /> : null}
                            {isBulkMode ? "Modo em Massa ON" : "Modo Individual"}
                        </Button>
                    </div>
                                ...
                                {history.map((v) => (
                                    <tr key={v.id} className={cn(
                                        ...
                                        <td className="p-4">
                                            {editingId === v.id ? (
                                                <div className="flex flex-col gap-1">
                                                     <label className="text-[7px] font-black uppercase text-zinc-500">Grupo</label>
                                                     <select
                                        ...
                                        <td className="p-4">
                                            {editingId === v.id ? (
                                                <div className="flex flex-col gap-1">
                                                    <label className="text-[7px] font-black uppercase text-zinc-500">Trocar Vendedor</label>
                                                    <select
                                                        value={editForm.user_id || ""}
                                                        onChange={e => setEditForm({ ...editForm, user_id: e.target.value })}
                                                        className="bg-zinc-900 border border-white/10 text-[10px] uppercase font-black text-amber-500 rounded px-2 py-1 outline-none"
                                                    >
                                                        {usuarios.map(u => (
                                                            <option key={u.id} value={u.id}>{u.discord_username}</option>
                                                        ))}
                                                    </select>
                                                </div>
                                            ) : (
                                                <span className="text-[10px] font-bold text-zinc-400 uppercase italic">
                                                    {usuarios.find(u => u.id === v.user_id)?.discord_username || "Desconhecido"}
                                                </span>
                                            )}
                                        </td>
                                        ...
                                        <td className="p-4 text-center">
                                            {editingId === v.id ? (
                                                <Input type="number" className="h-7 w-20 bg-zinc-900 text-xs border-white/10 text-center" value={editForm.capitulo} onChange={e => setEditForm({ ...editForm, capitulo: Number(e.target.value) })} />
                                            ) : (
                                                <span className="font-black text-zinc-100 italic text-xl">#{v.capitulo}</span>
                                            )}
                                        </td>
            ...
            {isBulkMode && selectedIds.size > 0 && (
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-zinc-950 border border-primary/30 shadow-2xl shadow-primary/20 rounded-2xl p-4 flex items-center gap-6 animate-in slide-in-from-bottom-8 z-50">
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-primary uppercase italic leading-none">{selectedIds.size} selecionados</span>
                        <span className="text-[8px] font-bold text-zinc-500 uppercase italic">Ações em massa</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <select 
                            onChange={(e) => handleBulkUpdate({ user_id: e.target.value })}
                            className="h-9 bg-zinc-900 border border-white/10 rounded-lg px-3 text-[10px] font-black uppercase italic text-amber-500 outline-none"
                        >
                        ...
                        <select 
                            onChange={(e) => handleBulkUpdate({ grupo_id: e.target.value })}
                            className="h-9 bg-zinc-900 border border-white/10 rounded-lg px-3 text-[10px] font-black uppercase italic text-primary outline-none"
                        >
                            <option value="">Trocar Grupo...</option>
                            <option value="null">Nenhum Grupo</option>
                            {gruposIniciais.map(g => <option key={g.id} value={g.id}>{g.nome}</option>)}
                        </select>

                        <div className="flex items-center gap-2">
                            <div className="relative">
                                <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[10px] font-bold text-zinc-500">$</span>
                                <Input 
                                    type="number" 
                                    step="0.01"
                                    placeholder="Valor"
                                    ...
                                    className="h-9 w-24 bg-zinc-900 border-white/10 pl-5 text-[10px] font-black uppercase italic text-emerald-500 outline-none"
                                />
                            </div>
                            <Button 
                                onClick={() => {
                                    if (bulkPrice && !isNaN(parseFloat(bulkPrice))) {
                                        handleBulkUpdate({ preco_unitario: parseFloat(bulkPrice) });
                                        setBulkPrice("");
                                    }
                                }}
                                disabled={!bulkPrice || isBulkUpdating}
                                className="h-9 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase text-[9px] px-3 rounded-lg transition-all"
                            >
                                Aplicar
                            </Button>
                        </div>
                        ...
                        <Button 
                            ...
                            variant="ghost" 
                            className="h-9 text-[10px] font-black uppercase italic text-zinc-500 hover:text-red-500 hover:bg-red-500/10"
                        >
