...
export async function PATCH(request: Request) {
    ...
    try {
        ...
        const { id, preco_unitario, capitulo, data_venda, grupo_id, user_id } = body;
        ...
        // Tratar erro de duplicidade (Unique Constraint)
        if (error.code === 'P2002') {
            return NextResponse.json({ 
                error: "Este capítulo já está registrado para este vendedor neste grupo. Verifique se há duplicidade." 
            }, { status: 400 });
        }
