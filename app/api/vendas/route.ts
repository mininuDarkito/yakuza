...
export async function GET(request: Request) {
  ...
  try {
    const vendas = await prisma.vendas.findMany({
      ...
      orderBy: [{ data_venda: "desc" }, { created_at: "desc" }],
    });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!vinculo) {
      return NextResponse.json(
        { error: "Vínculo com este grupo não encontrado ou inativo." },
        { status: 404 },
      );
    }

    // Priorizamos o preço unificado do grupo
    const precoUnitario = grupoPreco
      ? Number(grupoPreco.preco)
      : data.preco_unitario;
    ...
    const capsParaSalvar = data.capitulos.filter(
      (cap) => !capsExistentes.includes(cap),
    );
    ...
    const dataVendaFinal = data.data_venda
