...
export default async function NovaVendaPage(props: {
  ...
  if (isValidUuid(produto_id)) {
    ...
    if (grupoPadrao) {
      ...
      await sql.query(`
        INSERT INTO membros_grupo (user_id, grupo_id, created_at)
        VALUES ($1, $2, NOW())
        ON CONFLICT (user_id, grupo_id) DO NOTHING
      `, [userId, grupoPadrao])

      // Garante que a obra tenha um preço no grupo
      await sql.query(`
  ...
  const [gruposRes, produtosRes] = await Promise.all([
    ...
    sql.query(`
      SELECT 
        ...
        p.plataforma
      FROM produtos p
      INNER JOIN grupo_series gs ON p.id = gs.produto_id
