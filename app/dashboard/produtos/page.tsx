...
export default async function ProdutosPage() {
  ...
  const res = await sql.query(`
    ...
    INNER JOIN grupos g ON gs.grupo_id = g.id
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-black tracking-tighter uppercase italic text-white">
            Catálogo <span className="text-primary text-2xl">do Grupo</span>
          </h1>
          <p className="text-zinc-500 font-bold italic text-xs uppercase tracking-widest mt-1">
            Consulte as obras precificadas para os seus grupos de venda.
          </p>
        </div>
